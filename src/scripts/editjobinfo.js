document.addEventListener("DOMContentLoaded", function() {
    // Get all td elements
const tdElements = document.querySelectorAll('.table-fixed td');

// Loop through every third td element starting from the third one
for (let i = 2; i < tdElements.length; i += 3) {
    // Add contenteditable attribute
    tdElements[i].setAttribute('contenteditable', true);
    tdElements[i].classList.add("outline-none");
...
backarrow.addEventListener("click",(e)=>{
  ...
  window.location.href = "totalapplications.html"
