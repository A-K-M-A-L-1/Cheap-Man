...
const newJobListings = document.getElementsByClassName("newjob")

for (let i = 0; i < newJobListings.length; i++) {
  
  newJobListings[i].addEventListener("click", (e)=>{
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const totalVal = Number.parseInt(firstSec.children.length) + Number.parseInt(secondSec.children.length);

  Listings.innerText = totalVal;
});